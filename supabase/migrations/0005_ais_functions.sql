-- Unique constraint on mmsi required for ON CONFLICT (mmsi) in the upsert function.
-- Added because original schema only had imo as unique.
alter table public.vessels
  add constraint if not exists vessels_mmsi_key unique (mmsi);
...
-- Returns SETOF public.vessels to avoid OUT parameter name collisions with column names.
...
begin
  ...
  return query select * from public.vessels where id = v_id;
end;
$$;
